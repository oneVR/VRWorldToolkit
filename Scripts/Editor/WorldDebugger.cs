...
namespace VRWorldToolkit.WorldDebugger
{
    public class WorldDebugger : EditorWindow
    {
        ...
        System.Action RemoveRedundantLightProbes(LightProbeGroup[] lpgs)
        {
            return () =>
            {
                if (LightmapSettings.lightProbes != null)
                {
                    ...
                    if (EditorUtility.DisplayDialog("Remove redundant light probes?", "This operation will attempt to remove any redundant light probes in your scene. Bake your lighting before this operation to avoid any correct light probes getting removed. Do You want to continue?", "Yes", "Cancel"))
                    {
                        foreach (var lpg in lpgs)
                        {
        ...
        private readonly string noSceneDescriptor = "Your scene currently has no Scene Descriptor. Please add one yourself, or drag the VRCWorld prefab to your scene.";
        ...
        private readonly string mirrorWithDefaultLayers = "Your mirror %variable% has the default Reflect Layers set. Only having the layers you need enabled in mirrors can save a lot of frames especially in populated instances.";
        ...
        private readonly string combinedBrokenShader = "You have %count% materials using missing or broken shaders, these will just appear pink ingame.";
        ...
        private readonly string noLightmapUV = "You have a model \"%variable%\" set to be lightmapped that doesn't have Lightmap UVs. This causes issues when baking lighting. You can enable generating Lightmap UV's in the model's import settings.";
        private readonly string combineNoLightmapUV = "You have %count% models set to be lightmapped that don't have Lightmap UVs. This causes issues when baking lighting. You can enable generating Lightmap UV's in the model's import settings.";
        private readonly string lightsNotBaked = "Your scenes lighting is currently not baked. Consider baking your lights for improved performance.";
